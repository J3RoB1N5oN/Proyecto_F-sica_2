...
def seguimiento_automatico():
    global current_frame, current_frame_image, punto_previo_seguimiento, puntos_ciclo_actual
    
    # Validar que el color se haya definido (con 'k')
    if lower_white is None or upper_white is None:
        print("\nError: El rango de color no ha sido definido (presione 'k').")
        print("Ajuste la tolerancia y haga clic en la mancha blanca.")
        return

    print("\nIniciando seguimiento automático...")
    ...
    # Kernel para la limpieza morfológica
    ...
    for frame_num in range(total_frames):
        ...
        # --- ¡NUEVO! FILTRADO PROMEDIO (Mean Filter) ---
        # Aplicamos un suavizado (blur) al frame BGR original
        # con un kernel de 5x5 para reducir reflejos y ruido.
        frame_suavizado = cv2.blur(frame, (5, 5))
        # -----------------------------------------------

        # Convertimos el frame SUAVIZADO a HSV
        hsv_frame = cv2.cvtColor(frame_suavizado, cv2.COLOR_BGR2HSV)
        
        # Calculamos la máscara sobre el frame suavizado
        ...
        mejor_contorno = None
        distancia_minima = radio_promedio * 0.5 # Umbral de distancia (50% del radio)
        
        # Si hay contornos, buscar el más cercano al punto anterior
        if contours:
            if punto_previo_seguimiento is None:
                # Si es el primer frame, tomar el contorno más grande (o el primero)
                mejor_contorno = max(contours, key=cv2.contourArea)
            else:
                # Buscar el contorno más cercano al punto del frame anterior
                distancia_min_actual = float('inf')
                for c in contours:
                    M = cv2.moments(c)
                    if M["m00"] > 0:
                        cx = int(M["m10"] / M["m00"])
                        cy = int(M["m01"] / M["m00"])
                        dist = math.hypot(cx - punto_previo_seguimiento[0], cy - punto_previo_seguimiento[1])
                        
                        if dist < distancia_min_actual:
                            distancia_min_actual = dist
                            mejor_contorno = c
                
                # Opcional: Si el más cercano está muy lejos, descartarlo (quizás era ruido)
                if distancia_min_actual > distancia_minima:
                    mejor_contorno = None # Descartar por estar muy lejos

        # Si encontramos un contorno válido
        if mejor_contorno is not None:
            ...
            if M["m00"] > 0:
                ...
                # Calcular ángulo
                angulo_rad = math.atan2(cy - centro_rotacion[1], cx - centro_rotacion[0])
                
                # Guardar el punto y ángulo
                puntos_ciclo_actual[frame_num] = (cx, cy, angulo_rad)
                
                # Actualizar el punto previo para el siguiente frame
                punto_previo_seguimiento = (cx, cy)

                # --- Dibujar en el frame para visualización ---
                cv2.drawContours(frame, [mejor_contorno], -1, (0, 255, 255), 2)
                cv2.circle(frame, (cx, cy), 5, (0, 255, 0), -1)
                cv2.line(frame, centro_rotacion, (cx, cy), (255, 0, 0), 1)

        # Mostrar el frame procesado
        cv2.putText(frame, f"Procesando Frame: {frame_num}/{total_frames}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
        cv2.imshow('Seguimiento Automatico', frame)
        
        # (Pequeña pausa para que el video se renderice)
        if cv2.waitKey(1) & 0xFF == ord('q'):
             print("Seguimiento interrumpido por el usuario.")
             break

    # --- Fin del bucle ---
    
    cv2.destroyWindow('Seguimiento Automatico')
    print("Seguimiento automático completado.")
    
    # Volver al frame 0 en la ventana principal
    current_frame = 0
