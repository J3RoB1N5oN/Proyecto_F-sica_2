...
# --- Función de Seguimiento Automático (VERSIÓN 12: LÍMITE ESTRICTO) ---
def seguimiento_automatico():
    """
    Filtra TODOS los puntos para que solo queden los que están DENTRO
    del anillo, usando el radio_promedio como LÍMITE MÁXIMO ESTRICTO.
    """
    global clicked_points, puntos_ciclo_actual
    
    clicked_points = {}
    puntos_ciclo_actual = {}
    print("\n--- INICIANDO SEGUIMIENTO AUTOMÁTICO (V12: LÍMITE ESTRICTO) ---")
    ...
    # --- CAMBIO CLAVE: LÍMITE MÁXIMO ESTRICTO ---
    # Mantenemos un mínimo para ignorar el eje
    radio_minimo_permitido = radio_promedio * 0.7 
    # El máximo es AHORA exactamente el radio, sin márgenes.
    ...
    for frame_num in range(total_frames):
        ...
        if contours:
            ...
            # 1. Pre-filtrar contornos por área y posición general
            for c in contours:
            ...
            if contornos_candidatos:
                # 2. Agrupamos TODOS los puntos de TODOS los contornos candidatos
                all_points = np.concatenate(contornos_candidatos)
                
                # 3. Calcular la distancia al centro para CADA PUNTO
                distances = np.linalg.norm(all_points[:, 0] - centro_rotacion, axis=1)
                
                # 4. Crear un filtro (máscara) para los puntos DENTRO del anillo
                mask_min = distances >= radio_minimo_permitido
                # El filtro máximo AHORA ES ESTRICTO
                mask_max = distances <= radio_maximo_permitido 
                mask_anillo = mask_min & mask_max 
                
                # 5. Aplicar la máscara para quedarnos solo con los puntos buenos
                puntos_validos = all_points[mask_anillo]
                distancias_validas = distances[mask_anillo]

                # 6. Si hay puntos en el anillo, encontrar el más a la DERECHA
                if len(puntos_validos) > 0:
                    indice_punto_estela = np.argmax(puntos_validos[:, 0, 0])
                    
                    cx, cy = tuple(puntos_validos[indice_punto_estela][0])
    ...
    if len(clicked_points) < 2:
        print("ADVERTENCIA: No se detectaron suficientes puntos. Redefina los bordes ('b') con MÁS PRECISIÓN sobre la estela.")
    else:
...
# --- Función para dibujar todo en el frame (ACTUALIZADA V11) ---
def show_frame_with_markers(frame, frame_number):
    ...
    # Si todos los puntos están definidos, dibuja los radios y el ANILLO
    if radio_promedio > 0 and centro_rotacion:
        ...
        # --- MODIFICADO: DIBUJAR UN SOLO SEMICÍRCULO ---
        # Dibuja el semicírculo basado EXACTAMENTE en el radio_promedio
